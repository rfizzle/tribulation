...
description: Thread-safe patterns for shared-state managers, caches, and hot-reloadable registries in Fabric Minecraft mods. TRIGGER when creating or editing *Manager.java, *Cache.java, or any mod class using ConcurrentHashMap, volatile fields, AtomicReference, ThreadLocal, or synchronized blocks to manage shared mutable state.
---

The user is writing or modifying a shared-state manager in a Fabric mod — classes like FollowManager, NameManager, TradesManager, TextureCache, etc. These classes hold static or singleton state accessed from multiple threads (server tick, netty, reload). Apply this guidance to prevent the concurrency bugs that ConcurrentHashMap alone does not prevent.
